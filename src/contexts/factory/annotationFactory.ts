...
import React, { useCallback, useMemo, ReactNode, createContext, useContext } from 'react';
...
AnnotationContext.displayName = 'Annotation';
 ...
 */
function AnnotationProvider({ children }: AnnotationProviderProps) {
...
/**
 * Creates the annotation context using the factory pattern
 */
export function createAnnotationContext() {
  // Create action creators
  const actionCreators = {
    addAnnotation: (annotation: DrawingPath) => 
      ({ type: 'ADD_ANNOTATION', payload: { annotation } } as const),
    
    clearAnnotations: (clearTime: TimelinePosition) => 
      ({ type: 'CLEAR_ANNOTATIONS', payload: { clearTime } } as const),
    
    setAnnotations: (annotations: DrawingPath[]) => 
      ({ type: 'SET_ANNOTATIONS', payload: { annotations } } as const),
    
    setColor: (color: Color) => 
      ({ type: 'SET_COLOR', payload: { color } } as const),
    
    setWidth: (width: StrokeWidth) => 
      ({ type: 'SET_WIDTH', payload: { width } } as const),
    
    setEnabled: (enabled: boolean) => 
      ({ type: 'SET_ENABLED', payload: { enabled } } as const),
    
    setReplaying: (replaying: boolean) => 
      ({ type: 'SET_REPLAYING', payload: { replaying } } as const),
    
    reset: () => ({ type: 'RESET' } as const),
  };
  
  // Create the context
  const context = createReducerContext<AnnotationState, AnnotationAction>(
    'Annotation',
    annotationReducer,
  ...
  // Drawing options hook
  const useDrawingOptions = () => {
    const { state, setColor, setWidth } = useAnnotationExtended();
    return {
      color: state.currentColor,
      width: state.currentWidth,
      setColor,
  ...
  return {
    Provider,
    useAnnotation: useAnnotationExtended,
    useDrawingOptions,
