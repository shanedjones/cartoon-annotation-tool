...
import React, { useRef, useCallback, useMemo, useEffect, MutableRefObject, ReactNode, createContext, useContext, useReducer } from 'react';
...
const initialVideoContextState: VideoContextState = {
  ...
  hasError: false,
  errorMessage: '',
  videoRef: { current: null } as MutableRefObject<HTMLVideoElement | null>,
  isReady: false,
  cacheStats: {
    count: 0,
    totalSize: 0,
...
// Video reducer
function videoReducer(state: VideoContextState, action: VideoAction): VideoContextState {
  switch (action.type) {
    case 'PLAY':
      return {
        ...state,
 ...
 */
function VideoProvider({ children, initialUrl = '' }: VideoProviderProps) {
...
/**
 * Creates the video context using the factory pattern
 */
export function createVideoContext() {
  ...
  const actionCreators = {
    ...
    seek: (time: number) => 
      ({ type: 'SEEK', payload: { time } } as const),
    
    updateTime: (time: number) => 
      ({ type: 'UPDATE_TIME', payload: { time } } as const),
    
    updateDuration: (duration: number) => 
      ({ type: 'UPDATE_DURATION', payload: { duration } } as const),
    
    setPlaybackRate: (rate: number) => 
      ({ type: 'SET_PLAYBACK_RATE', payload: { rate } } as const),
    
    setDimensions: (dimensions: Dimensions) => 
      ({ type: 'SET_DIMENSIONS', payload: { dimensions } } as const),
    
    setMuted: (muted: boolean) => 
      ({ type: 'SET_MUTED', payload: { muted } } as const),
    
    setVolume: (volume: number) => 
      ({ type: 'SET_VOLUME', payload: { volume } } as const),
    
    setVideoUrl: (url: string) => 
      ({ type: 'SET_VIDEO_URL', payload: { url } } as const),
    
    setCachedUrl: (url: string | null) => 
      ({ type: 'SET_CACHED_URL', payload: { url } } as const),
    
    setLoading: (isLoading: boolean) => 
      ({ type: 'SET_LOADING', payload: { isLoading } } as const),
    
    setError: (hasError: boolean, errorMessage: string) => 
      ({ type: 'SET_ERROR', payload: { hasError, errorMessage } } as const),
    
    setReady: (ready: boolean) => 
      ({ type: 'SET_READY', payload: { ready } } as const),
    
    setCacheStats: (stats: { count: number; totalSize: number }) => 
      ({ type: 'SET_CACHE_STATS', payload: { stats } } as const),
    
    reset: () => ({ type: 'RESET' } as const),
  };
  
  // Create the context
  const context = createReducerContext<VideoContextState, VideoAction>(
    'Video',
    videoReducer,
    initialVideoContextState,
    actionCreators
  ...
  const { Provider: BaseProvider, useContextHook } = context;
  
  // Create custom Provider with videoRef
  const Provider = ({ children, initialUrl = '' }: { children: ReactNode, initialUrl?: string }) => {
    const videoRef = useRef<HTMLVideoElement | null>(null);
    
    return React.createElement(
      BaseProvider,
      { value: { videoRef, initialUrl } },
      children
    );
  };
  
  // Create custom hook with video-specific functionality
  const useVideoExtended = () => {
    ...
    // Format cache size
    const getFormattedCacheSize = useCallback(() => 
      formatFileSize(state.cacheStats.totalSize), [state.cacheStats.totalSize]);
    ...
    }, [state.playback.isPlaying, contextValue]);
    ...
    const resetCache = useCallback(async () => {
      try {
        await clearVideoCache();
        const stats = await getVideoCacheStats();
        contextValue.setCacheStats(stats);
      } catch (error) {
        console.error('Error clearing video cache:', error);
        contextValue.setError(true, 'Failed to clear video cache');
      }
    }, [contextValue]);
  ...
  const useVideoDimensions = () => {
    const { state, setDimensions } = useVideoExtended();
